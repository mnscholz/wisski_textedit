...
//        alert("Currently, Chrome is not supported by WissKI Annotation Tools. Some functionality might not work correctly.");
      ...
      ed.addCommand('wisskiCoreToggleApproved', function(ui, anno) {
        ...
        if (tinymce.activeEditor.dom.hasClass(anno, 'wisski_anno_approved')) {
          ...
          tinymce.activeEditor.controlManager.setActive('wisskiCoreToggleApproved', false);
        } else {
          ...
          tinymce.activeEditor.controlManager.setActive('wisskiCoreToggleApproved', true);
        }

      }, t);


      // Set/unset subject state. if anno is not given, takes current anno
      ed.addCommand('wisskiCoreToggleSubject', function(ui, anno) {
        var t = ed.plugins.wisskicore;
        if (!anno) {
          anno = t.current_anno;
          if (!anno) {
        ...
        if (tinymce.activeEditor.dom.hasClass(anno, 'wisski_anno_subject')) {
          tinymce.activeEditor.dom.removeClass(anno, 'wisski_anno_subject');
          tinymce.activeEditor.controlManager.setActive('wisskiCoreToggleSubject', false);
        } else {
          tinymce.activeEditor.dom.addClass(anno, 'wisski_anno_subject');
          tinymce.activeEditor.controlManager.setActive('wisskiCoreToggleSubject', true);
        }
      ...
      t.onAnnoChange.add(function(ed, cm, current_anno) {
        ...
        cm.setDisabled('wisskiCoreToggleSubject', current_anno == null);
        cm.setActive('wisskiCoreToggleSubject', (!current_anno) ? false : ed.dom.hasClass(current_anno, 'wisski_anno_subject'));
      });
      ...
      ed.addButton('wisskiCoreToggleSubject', {
        cmd : 'wisskiCoreToggleSubject',
        title : '(Un)mark as text subject/topic',
        image : url + '/img/subject.png',
      });
      ...
      ed.onRemove.add(function() {
        var l = ed.dom.select('.wisski_anno_subject');
        window.console.log(l);
        if (l.length == 0) return;

        var gidField = $('#edit-gid');
        var instField = $('#edit-inst');
        if (instField.length != 1 || gidField.length != 1) return;
        
        window.console.log(instField);
        window.console.log('gidf', gidField);
        var gid = ed.dom.getAttrib(gidField[0], 'value', '');
        window.console.log(gid);
        if (gid == '') return;
        window.console.log(gid);
        
        for (var i in l) {
          if (gid == t.getAnnoClass(l[i])) {
            
            ed.dom.setAttrib(instField[0], 'value', t.getAnnoURI(l[i]));
            break;
          }
        }
